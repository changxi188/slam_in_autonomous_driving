...
void FeatureExtraction::Extract(FullCloudPtr pc_in, CloudPtr pc_out_edge, CloudPtr pc_out_surf, FullCloudPtr valid_pc)
{
    ...
    for (std::size_t j = 0; j < horizen_size; ++j)
    {
        for (std::size_t i = 0; i < ground_scan_ind_; ++i)
        {
            ...
            // 如果俯仰角在10度以内，则判定(i,j)为地面点,ground_mat_[i][j]=1
            // 否则，则不是地面点，进行后续操作
    ...
    for (int i = 0; i < num_scans_; i++)
    {
        for (int j = 0; j < horizen_size; ++j)
        {
            if (ground_mat_.at<int8_t>(i, j) == 1)
            {
                PointType ground_point = scans_in_each_line[i]->at(j);
                pc_out_surf->push_back(ground_point);
            }
        }
    }
    return;
    */
    ...
    // 处理曲率
    std::vector<std::vector<IdAndValue>> cloud_curvatures;  // 每条线对应的曲率
    for (int i = 0; i < num_scans_; i++)
    {
        std::vector<IdAndValue> cloud_curvature;  // 每条线对应的曲率
        for (int j = 5; j < (int)scans_in_each_line[i]->points.size() - 5; j++)
        {
            if (scans_in_each_line[i]->points[j - 5].intensity == -1 ||
                scans_in_each_line[i]->points[j - 4].intensity == -1 ||
                scans_in_each_line[i]->points[j - 3].intensity == -1 ||
                scans_in_each_line[i]->points[j - 2].intensity == -1 ||
                scans_in_each_line[i]->points[j - 1].intensity == -1 ||
                scans_in_each_line[i]->points[j + 5].intensity == -1 ||
                scans_in_each_line[i]->points[j + 4].intensity == -1 ||
                scans_in_each_line[i]->points[j + 3].intensity == -1 ||
                scans_in_each_line[i]->points[j + 1].intensity == -1 ||
                scans_in_each_line[i]->points[j].intensity == -1)
            {
                IdAndValue distance(j, -1);
        ...
        cloud_curvatures.push_back(cloud_curvature);
    }

    for (int i = 0; i < num_scans_; i++)
    {
        std::vector<IdAndValue> cloud_curvature = cloud_curvatures.at(i);  // 每条线对应的曲率
        int                     total_points    = cloud_curvature.size();
        ...
        for (int j = 0; j < 6; j++)
        {
            ...
            ExtractFromSector(i, scans_in_each_line[i], sub_cloud_curvature, pc_out_edge, pc_out_surf);
        }
    }
}

void FeatureExtraction::ExtractFromSector(const int scan_id, const CloudPtr& pc_in,
                                          ...
                                          CloudPtr& pc_out_surf)
{
    ...
    for (int i = 0; i <= (int)cloud_curvature.size() - 1; i++)
    {
            ...
            ground_mat_.at<int8_t>(scan_id, ind) == 1)
        {
            ...
            for (int l = 1; l <= 5; ++l)
            {
                double diffX = pc_in->points[ind + l].x - pc_in->points[ind + l - 1].x;
                double diffY = pc_in->points[ind + l].y - pc_in->points[ind + l - 1].y;
                double diffZ = pc_in->points[ind + l].z - pc_in->points[ind + l - 1].z;
                if (diffX * diffX + diffY * diffY + diffZ * diffZ > 0.05)
                {
                    break;
                }
                picked_points.push_back(ind + l);
            }
